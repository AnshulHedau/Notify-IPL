...
@app.route("/")
def index():
    return_value = {"message": "Welcome to the Notify-IPL API 2019!"}
...
def score():
    status = 0
    team = ["CSK", "DD", "KXIP", "KKR", "MI", "RR", "RCB", "SRH","IND","AFG","SL","WI","ENG","ZIM","PAK","RSA"]
    team_name = ["Chennai Super Kings", "Delhi Daredevils", "Kings XI Punjab", "Kolkata Knight Riders",
                 "Mumbai Indians", "Rajasthan Royals", "Royal Challengers Bangalore", "Sunrisers Hyderabad","India",
                 "Afganistan","Sri Lanka","Windies","England","Zimbabwe","Pakistan","South Africa"]
    images = ["https://iplstatic.s3.amazonaws.com/players/284/1.png",
             ...
             "http://internationalplayers.s3.amazonaws.com/284/182.png"]
    page = requests.get("http://www.cricbuzz.com/cricket-match/live-scores")
    soup = BeautifulSoup(page.content, 'html.parser')
    productrow = soup.find(class_="cb-schdl").contents[2]
    forecast_items = productrow.find(class_="cb-mtch-lst")
